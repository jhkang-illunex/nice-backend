...
def test_read_csv_with_rename(tmp_path: Path) -> None:
    ...
    df = _read_csv(csv, rename={"기업ID": "firm_id", "기업명": "firm_name"},
                   columns=None, delimiter=",", encoding="utf-8")
...
def test_read_csv_required_columns_missing(tmp_path: Path) -> None:
    ...
    with pytest.raises(KeyError, match="missing required columns"):
        _read_csv(csv, rename=None, columns=["a", "c"],
...
def test_read_csv_columns_filter_and_reorder(tmp_path: Path) -> None:
    csv = tmp_path / "in.csv"
    _write(csv, "b,a,c\n2,1,3\n")
    df = _read_csv(csv, rename=None, columns=["a", "b"],
                   delimiter=",", encoding="utf-8")
...
def test_dry_run_does_not_call_sink(tmp_path: Path) -> None:
    ...
    report = upload_to_pg(
        csv, table="firms", pk=["firm_id"], dry_run=True, sink=_BoomSink(),  # type: ignore[arg-type]
    )
